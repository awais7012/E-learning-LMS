...
const ManageCourse = () => {
  ...
  useEffect(() => {
    const fetchCourseData = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:8000/api/courses/${courseId}/manage`, {
          headers: {
  ...
  // Fetch certificate data when certificate tab is active
  useEffect(() => {
    if (activeTab === "certificates" && courseId) {
  ...
  const fetchCertificateData = async () => {
    try {
      const token = localStorage.getItem("accessToken")
      const response = await axios.get(`http://localhost:8000/api/certificates/course/${courseId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })

      setCertificateTemplate(response.data.certificate_template)
      setIssuedCertificates(response.data.issued_certificates || [])

      // Pre-fill the form if a certificate template exists
      if (response.data.certificate_template) {
        setNewCertificate({
          title: response.data.certificate_template.title,
          description: response.data.certificate_template.description || "",
        })
      }
    } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      const token = localStorage.getItem("accessToken")
      await axios.put(`http://localhost:8000/api/courses/${courseId}/manage`, course, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
  ...
  const submitLesson = async () => {
    ...
    try {
      ...
      setModules(
        modules.map((module) =>
          module._id === currentModuleId ? { ...module, lessons: [...module.lessons, response.data.lesson] } : module,
        ),
      )
  ...
  const submitNewModule = async () => {
    ...
    try {
      ...
      const response = await axios.post(`http://localhost:8000/api/courses/${courseId}/modules`, formData, {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "multipart/form-data",
        },
  ...
  const submitCertificateTemplate = async () => {
    ...
    try {
      const token = localStorage.getItem("accessToken")
      const formData = new FormData()
      formData.append("title", newCertificate.title.trim())
      formData.append("course_id", courseId)
      if (newCertificate.description) {
        formData.append("description", newCertificate.description)
      }

      const response = await axios.post(`http://localhost:8000/api/certificates/create`, formData, {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "multipart/form-data",
        },
      ...
      setCertificateTemplate(response.data.certificate)
      setIsCertificateModalOpen(false)

      // Update course state to reflect certificate is offered
      setCourse((prev) => ({
        ...prev,
        certificateOffered: true,
        certificateTitle: newCertificate.title,
        certificateDescription: newCertificate.description,
      }))
  ...
  const issueCertificate = async () => {
    ...
    try {
      const token = localStorage.getItem("accessToken")
      const response = await axios.post(
        `http://localhost:8000/api/certificates/issue/${courseId}/${selectedStudentId}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      setIssuedCertificates([...issuedCertificates, response.data.certificate])
  ...
  // Function to check if a student already has a certificate
  const hasStudentCertificate = (studentId) => {
    return issuedCertificates.some((cert) => cert.student_id === studentId)
  }
    ...
    <div className="w-full">
      {isLoading ? (
        ...
        <div className="bg-white rounded-xl shadow-sm">
          {/* Main content area */}
          <div className="p-6">
            <h1 className="text-2xl font-bold mb-6">Manage Course: {course.courseName}</h1>
              ...
              <div>
                ...
                <div className="mb-8">
                  ...
                  <div className="bg-gray-50 border border-gray-200 rounded-lg overflow-hidden">
                    {modules.map((module) => (
                      <div key={module._id} className="border-b border-gray-200 last:border-b-0">
                        <div
                            ...
                            {module.lessons.length > 0 ? (
                              <ul className="divide-y divide-gray-100">
                                {module.lessons.map((lesson) => (
                                  <li key={lesson._id} className="py-3 flex justify-between items-center">
                                    <div className="flex items-center">
                                    ...
                                    <div className="flex space-x-1">
                                      <button
                                        ...
                                        onClick={(e) => {
                                          e.stopPropagation()
                                          openEditLessonModal(module._id, lesson)
                                        }}
                                      >
                                        <FiEdit size={15} />
                                      </button>
                ...
                {certificateTemplate ? (
                  <div className="space-y-6">
                    <div className="bg-white p-6 border border-gray-200 rounded-lg">
                      <div className="flex justify-between items-start mb-4">
                        <div>
                          <h3 className="text-lg font-medium text-gray-900">{certificateTemplate.title}</h3>
                          <p className="text-sm text-gray-500 mt-1">
                            {certificateTemplate.description || "No description provided"}
                          </p>
                        </div>
                        <button
                      ...
                      <div className="mt-4 p-4 bg-gray-50 rounded-lg border border-dashed border-gray-300">
                        <div className="flex justify-center">
                          <div className="relative w-full max-w-md aspect-[1.4/1] bg-white border-2 border-gray-300 rounded-lg overflow-hidden">
                            <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center">
                              <FiAward className="text-[#19a4db] w-16 h-16 mb-4" />
                              <h4 className="text-xl font-bold text-[#19a4db] mb-2">Certificate of Completion</h4>
                              <p className="text-lg font-medium">{certificateTemplate.title}</p>
                              <div className="mt-4 w-3/4 border-t border-gray-300 pt-4">
                                <p className="text-sm text-gray-500">This certificate is awarded to</p>
                                <p className="text-lg font-medium italic mt-1">Student Name</p>
                              </div>
                              <div className="mt-4 text-sm text-gray-500">
                                <p>for successfully completing the course</p>
                                <p className="font-medium mt-1">{course.courseName}</p>
                              </div>
                            </div>
                          </div>
                        </div>
                        <p className="text-xs text-center text-gray-500 mt-4">
                          This is a preview of how the certificate will look. The actual certificate will include
                          student name and issue date.
                        </p>
                      </div>

                      <div className="mt-6">
                        <div className="flex justify-between items-center mb-4">
                          <h4 className="font-medium">Issued Certificates</h4>
                          <button
                              ...
                              <tbody className="bg-white divide-y divide-gray-200">
                                {issuedCertificates.map((cert) => (
                                  <tr key={cert._id}>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                      <div className="text-sm text-gray-500">{cert.credential_id}</div>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                      <a
                                        href={`http://localhost:8000/uploads/${cert.certificate_url}`}
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="text-[#19a4db] hover:underline flex items-center"
                                      >
                  ...
                  <div className="bg-gray-50 p-6 rounded-lg text-center">
                    ...
                    <h3 className="text-lg font-medium text-gray-900 mb-1">
                      {course.certificateOffered ? "Certificate Template Not Created" : "Certificate Not Enabled"}
                    </h3>
                    <p className="text-gray-500 mb-4">
                      {course.certificateOffered
                        ? "Create a certificate template to start issuing certificates to your students."
                        : "Enable certificates in the Basic Info tab to configure them."}
                    </p>
                    {course.certificateOffered ? (
                      <button
                        type="button"
            ...
            <div className="flex space-x-3">
              ...
              <button
                ...
                className={`px-6 py-2 bg-[#19a4db] text-white rounded-lg flex items-center ${
                  isSaving ? "opacity-70 cursor-not-allowed" : "hover:bg-[#1582af]"
                }`}
                disabled={isSaving}
              >
                {isSaving ? (
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg max-w-lg w-full">
            <div className="border-b border-gray-200 px-6 py-4 flex justify-between items-center">
              ...
              <button onClick={() => setIsModuleModalOpen(false)} className="text-gray-400 hover:text-gray-500">
                <FiX size={20} />
              </button>
            </div>
            ...
            <div className="bg-gray-50 px-6 py-4 flex justify-end space-x-3 rounded-b-lg">
              ...
              <button type="button" onClick={submitNewModule} className="px-4 py-2 bg-[#19a4db] text-white rounded-lg">
                Add Module
              </button>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg max-w-lg w-full">
            <div className="border-b border-gray-200 px-6 py-4 flex justify-between items-center">
              ...
              <button onClick={() => setIsMaterialModalOpen(false)} className="text-gray-400 hover:text-gray-500">
                <FiX size={20} />
              </button>
            </div>
      ...
      {isLessonModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg max-w-lg w-full">
            <div className="border-b border-gray-200 px-6 py-4 flex justify-between items-center">
              <h3 className="text-lg font-medium">{isEditingLesson ? "Edit Lesson" : "Add New Lesson"}</h3>
              <button onClick={() => setIsLessonModalOpen(false)} className="text-gray-400 hover:text-gray-500">
                <FiX size={20} />
              </button>
            </div>
            ...
            <div className="px-6 py-4">
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Lesson Material</label>
                      ...
                      <div className="mt-2">
                        <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-lg">
                          <div className="space-y-1 text-center">
                            ...
                            <p className="text-xs text-gray-500">PDF, Video, Documents up to 100MB</p>
                            {newLesson.material && (
                              <p className="text-sm text-gray-700 font-medium mt-2">{newLesson.materialName}</p>
                            )}
                          </div>
                        </div>
                      </div>
            ...
            <div className="bg-gray-50 px-6 py-4 flex justify-end space-x-3 rounded-b-lg">
              ...
              <button type="button" onClick={submitLesson} className="px-4 py-2 bg-[#19a4db] text-white rounded-lg">
          ...
          <div className="bg-white rounded-lg max-w-lg w-full">
            <div className="border-b border-gray-200 px-6 py-4 flex justify-between items-center">
              <h3 className="text-lg font-medium">
                {certificateTemplate ? "Edit Certificate Template" : "Create Certificate Template"}
              </h3>
              <button onClick={() => setIsCertificateModalOpen(false)} className="text-gray-400 hover:text-gray-500">
                <FiX size={20} />
              </button>
            </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  <p className="text-sm text-gray-500 italic">
                    Note: A default certificate template will be used. Custom template uploads will be available in a
                    future update.
                  </p>
                </div>
              </div>
      ...
      {isIssueCertificateModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg max-w-lg w-full">
            <div className="border-b border-gray-200 px-6 py-4 flex justify-between items-center">
              <h3 className="text-lg font-medium">Issue Certificate</h3>
              <button
            ...
            <div className="px-6 py-4">
              <div className="space-y-4">
                <p className="text-sm text-gray-600">
                  Select a student to issue a certificate for completing this course.
                </p>
                ...
                <div>
                  ...
                  >
                    ...
                    {enrolledStudents.map((student) => (
                      <option key={student._id} value={student._id} disabled={hasStudentCertificate(student._id)}>
                        {student.full_name || student.username}
                        {hasStudentCertificate(student._id) ? " (Certificate already issued)" : ""}
                      </option>
                    ))}
                  </select>
                </div>
            ...
            <div className="bg-gray-50 px-6 py-4 flex justify-end space-x-3 rounded-b-lg">
              ...
              <button
                ...
                className="px-4 py-2 bg-[#19a4db] text-white rounded-lg flex items-center"
                disabled={issuingCertificate || !selectedStudentId}
              >
                {issuingCertificate ? (
                  <>
                    <svg
